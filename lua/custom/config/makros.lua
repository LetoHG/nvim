...
-- vim.fn.setreg('t', 'iTODO(letohg): [' .. esc .. ":put =strftime('%Y-%m-%d')" .. cr .. 'i] ' .. esc .. 'gccA')
vim.fn.setreg('t', 'iTODO(letohg): []' .. esc .. ':InsertDate' .. cr .. esc .. 'gccA ')
...
local function insert_note(category)
  ...
  local current_line = vim.api.nvim_get_current_line()
  local date_str = os.date '%Y-%m-%d'
  local new_line = current_line:sub(1, col) .. ' // ' .. category .. '(letohg): [' .. date_str .. '] ' .. current_line:sub(col + 1)
  vim.api.nvim_set_current_line(new_line)
end
